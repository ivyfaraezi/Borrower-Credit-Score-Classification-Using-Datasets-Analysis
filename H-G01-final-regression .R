# =========================================================
# FINAL PROJECT – OPTION 1 (Regression)
...
for (col in numeric_cols) {
  ...
  # Count how many values fall outside bounds
...
# Apply encoding to all categorical predictors
for (col in categorical_features) {
...
# Identify numeric columns with non-zero variance
numeric_cols <- names(data_clean)[sapply(data_clean, is.numeric)]
...
# ============================================================
# LINEAR REGRESSION MODELING – BALANCE PREDICTION
# ============================================================

# ------------------------------------------------------------
# Objective:
# To build a Linear Regression model that predicts the
# continuous variable 'balance' and evaluates its performance
# using standard regression metrics.
# ------------------------------------------------------------


# Ensure target variable 'balance' is numeric
data_clean$balance <- as.numeric(data_clean$balance)

# Remove rows with missing values (important for regression)
...
RMSE <- sqrt(mean((predictions - test_data$balance)^2))

# Mean Absolute Error (MAE)
MAE <- mean(abs(predictions - test_data$balance))
...
ggplot(
  ...
  theme_minimal() +
  ggtitle("Actual vs Predicted Balance (Linear Regression)") +
  labs(
    x = "Actual Balance",
    y = "Predicted Balance"
  )
