...
# maxdepth limits tree complexity to avoid overfitting
dt_model <- rpart(
  y ~ .,
  data = train_data,
  method = "class",
...
rpart.plot(dt_model, main = "Decision Tree (Classification)")
...
conf_mat <- confusionMatrix(predictions, test_data$y)
conf_mat
